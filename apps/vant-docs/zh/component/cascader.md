...
- `change` 事件保留逐级选择时的中间态，适合做异步加载
- 真正写回字段的是叶子节点触发后的 `update:modelValue`
- `change` / `finish` 的 payload 都会额外暴露当前 `field` 和当前路径值 `currentValue`
...
| `change`            | 任意层级切换时触发         | ^[Function]`(payload: { value, currentValue, tabIndex, selectedOptions, field? }) => void` |
| `finish`            | 选中叶子节点时触发         | ^[Function]`(payload: { value, currentValue, tabIndex, selectedOptions, field? }) => void` |
| `open`              | 弹层打开时触发             | `-`                                                                                        |
| `close`             | 弹层关闭时触发             | `-`                                                                                        |
| `opened`            | 弹层打开且动画结束后触发   | `-`                                                                                        |
| `closed`            | 弹层关闭且动画结束后触发   | `-`                                                                                        |
