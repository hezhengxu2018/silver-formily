...
- Popup 壳层配置统一通过 `popupProps` 传入，避免和 DatePicker 自身参数混在一起
- 字段值统一保存为字符串，例如 `'2026-03-30'`
...
当前封装内部固定包了一层 `Popup`，以下弹层属性需要通过 `popupProps` 传入，目前的默认值为：
...
- [Vant DatePicker 官方文档（正式站）](https://vant-ui.github.io/vant/#/zh-CN/date-picker)
