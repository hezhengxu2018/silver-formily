...
`@silver-formily/vue` no longer re-exports `Schema`. When the Vue API mentions `Schema`, `ISchema`, `x-reactions`, or `x-component-props`, those contracts come from `@silver-formily/json-schema`.
...
::: tip Migration Note
Starting with `3.x`, every schema-related snippet in `vue-docs` now uses `@silver-formily/json-schema`. The `@silver-formily/vue` docs only explain how Vue consumes schema objects and do not duplicate the JSON Schema protocol reference.
:::
