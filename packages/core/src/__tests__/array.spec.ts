...
it('array field children state exchanges', () => {
...
it('lazy array field query each', () => {
  const form = attach(createForm())
  const array = attach(
    form.createArrayField({
      name: 'array',
...
it('array field move api with children', async () => {
  const form = attach(createForm())
  attach(
    form.createField({
      name: 'other',
...
it('array field remove memo leak', async () => {
  const handler = vi.fn()
...
it('incomplete insertion of array elements', async () => {
  const form = attach(
    createForm({
      values: {
...
it('void array items need skip data', () => {
  const form = attach(createForm())
  const array = attach(
    form.createArrayField({
      name: 'array',
...
it('array field remove can not memory leak', async () => {
  const handler = vi.fn()
  const form = attach(
    createForm({
      values: {
...
it('array field patch values', async () => {
  const form = attach(createForm())
...
it('array remove with initialValues', async () => {
  const form = attach(
    createForm({
      initialValues: {
        array: [{ a: 1 }, { a: 2 }],
...
it('record: find array nest field record', () => {
  const form = attach(
    createForm({
      initialValues: {
...
it('record: find array field record', () => {
  const form = attach(
    createForm({
      initialValues: {
        array: [1, 2, 3],
...
it('record: find object field record', () => {
  const form = attach(
    createForm({
      initialValues: {
        a: {
