import { batch, observable } from '@silver-formily/reactive'
...
it('createField/createArrayField/createObjectField/createVoidField', () => {
  const form = attach(createForm())
  const normal = attach(
    form.createField({
      name: 'normal',
...
it('no field initialValues merge', () => {
  const form = attach(
    createForm<any>({
      values: {
        aa: '123',
...
it('observable values/initialValues', () => {
...
it('deleteValuesIn/deleteInitialValuesIn', () => {
  const form = attach(
    createForm<{
      aa?: number
      bb?: number
...
it('setSubmitting/setValidating', async () => {
  const form = attach(createForm())
...
it('setEffects/addEffects/removeEffects', () => {
  const form = attach(createForm())
...
it('query', () => {
  ...
  const objectVoid = form.query('object.void').take() as any
  const arrayField = form.query('array').take() as any
  expect(objectVoid?.value).toBeUndefined()
  expect(objectVoid?.initialValue).toBeUndefined()
  expect(objectVoid?.inputValue).toBeUndefined()
  expect(arrayField?.value).toEqual([])
  expect(arrayField?.initialValue).toBeUndefined()
  expect(arrayField?.inputValue).toBeNull()
  ;(form as any).setFieldState('array', {
    value: [111],
    initialValue: [111],
    inputValue: [111],
  })
  const updatedArrayField = form.query('array').take() as any
  expect(updatedArrayField?.value).toEqual([111])
  expect(updatedArrayField?.initialValue).toEqual([111])
  expect(updatedArrayField?.inputValue).toEqual([111])
  expect(form.query('array').getIn('inputValue')).toEqual([111])
  expect((form.query('opo').take() as any)?.value).toBeUndefined()
  expect(form.query('opo').getIn('value')).toBeUndefined()
  expect((form.query('opo').take() as any)?.initialValue).toBeUndefined()
  expect((form.query('opo').take() as any)?.inputValue).toBeUndefined()
})

it('notify/subscribe/unsubscribe', () => {
  const form = attach(createForm())
...
it('setState/getState/setFormState/getFormState/setFieldState/getFieldState', () => {
  const form = attach(createForm())
...
it('validate/valid/invalid/errors/warnings/successes/clearErrors/clearWarnings/clearSuccesses/queryFeedbacks', async () => {
  const form = attach(createForm())
  const aa = attach(
    form.createField({
      name: 'aa',
...
it('setPattern/pattern/editable/readOnly/disabled/readPretty', () => {
  const form = attach(
    createForm({
      pattern: 'disabled',
    }),
...
it('setDisplay/display/visible/hidden', () => {
  const form = attach(
    createForm({
      display: 'hidden',
    }),
...
it('submit', async () => {
  ...
  let errors1: Error | undefined
  try {
    await form.submit(onSubmit)
  }
  catch (e) {
    errors1 = e instanceof Error ? e : new Error(String(e))
  }
  ...
  try {
    ...
    errors2 = e instanceof Error ? e : new Error(String(e))
  }
...
it('initialValues merge values before create field', () => {
  const form = attach(createForm())
  const array = attach(
    form.createArrayField({
      name: 'array',
...
it('no patch with empty initialValues', () => {
  const form = attach(
    createForm({
      values: {
        array: [1, 2, 3],
...
it('initialValues merge values after create field', () => {
  const form = attach(createForm())
  const aa = attach(
    form.createArrayField({
      name: 'aa',
...
it('remove property of form values with undefined value', () => {
  const form = attach(createForm())
  const field = attach(
    form.createField({
      name: 'aaa',
  ...
  field.display = 'none'
  expect(Object.hasOwn(form.values, 'aaa')).toBeFalsy()
  field.display = 'visible'
  expect(Object.hasOwn(form.values, 'aaa')).toBeTruthy()
  field.setValue(undefined)
  expect(Object.hasOwn(form.values, 'aaa')).toBeTruthy()
})

it('empty array initialValues', () => {
  const form = attach(
    createForm({
      initialValues: {
        aa: [0],
...
it('form lifecycle can be triggered after call form.setXXX', () => {
...
it('form values change with array field(default value)', async () => {
  const handler = vi.fn()
  const form = attach(
    createForm({
      effects() {
...
it('validate will skip display none', async () => {
...
it('validate will skip unmounted', async () => {
  const validateA = vi.fn()
  const validateB = vi.fn()
  const form = attach(
    createForm({
...
it('validate will skip uneditable', async () => {
  const validateA = vi.fn()
  const validateB = vi.fn()
  const form = attach(
    createForm({
...
it('validator order with format', async () => {
  const form = attach(createForm())
  ...
  expect(results.map((result: { messages: string[] }) => result.messages)).toEqual([
    ['The field value is required'],
    ['The field value is required'],
  ])
})

it('form unmount can not effect field values', () => {
  const form = attach(
    createForm({
      values: {
        aa: '123',
...
it('form clearFormGraph need clear field values', () => {
  const form = attach(
    createForm({
      values: {
        aa: '123',
...
it('form clearFormGraph not clear field values', () => {
  const form = attach(
    createForm({
      values: {
        aa: '123',
...
it('form values auto clean with visible false', () => {
  const form = attach(
    createForm({
      initialValues: {
        aa: '123',
...
it('form values auto clean with visible false in async setInitialValues', () => {
  const form = attach(createForm())
  attach(
    form.createField({
      name: 'aa',
...
it('form values ref should not changed with setValues', () => {
  const form = attach(
    createForm({
      values: {
        aa: '123',
...
it('form initial values ref should not changed with setInitialValues', () => {
  const form = attach(
    createForm({
      initialValues: {
        aa: '123',
...
it('form query undefined query should not throw error', () => {
  const form = attach(createForm())
