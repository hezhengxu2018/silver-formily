...
export class BaseField<Decorator = any, Component = any, TextType = any> {
  ...
  get indexes(): number[] {
    return this.path.transform(/^\d+$/, (...args) =>
      args.map(index => Number(index))) as number[]
  }
  ...
  setComponent = <C extends JSXComponent, ComponentProps extends object = Record<string, unknown>>(
    component?: C,
  ...
  setComponentProps = <ComponentProps extends object = Record<string, unknown>>(
    props?: ComponentProps,
  ) => {
    if (props) {
      this.componentProps = this.componentProps || {}
  ...
  setDecorator = <D extends JSXComponent, ComponentProps extends object = Record<string, unknown>>(
    component?: D,
    props?: ComponentProps,
  ) => {
    if (component) {
  ...
  setDecoratorProps = <ComponentProps extends object = Record<string, unknown>>(
    props?: ComponentProps,
  ) => {
    if (props) {
      this.decoratorProps = this.decoratorProps || {}
