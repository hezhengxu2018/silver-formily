...
import { action, batch, define, observable, toJS } from '@silver-formily/reactive'
import {
  isArr,
  isEmpty,
  isValid,
...
> extends BaseField<Decorator, Component, TextType> {
  ...
  get required() {
    ...
    return validators.some((desc) => {
      return !!desc && typeof desc === 'object' && 'required' in desc && !!desc.required
    })
  }
