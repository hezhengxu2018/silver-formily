...
export function createEffectContext<T = any>(defaultValue?: T) {
  let index: number
  return {
    provide(value?: T) {
      if (GlobalState.effectStart) {
...
export function runEffects<Context>(context?: Context, ...args: ((context: Context) => void)[]): LifeCycle[] {
