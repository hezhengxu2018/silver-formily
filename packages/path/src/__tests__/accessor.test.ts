...
it('test setIn array properties', () => {
  const value = { array: [] }
  setIn(value, 'array.xxx', 'hello world')
  expect(value).toEqual({ array: [] })
})

it('test setIn dose not affect other items', () => {
  const value = {
    aa: [
      {
        dd: [
...
it('getIn with a.b.c.{aaa,bbb} source has extra property', () => {
  expect(
    Path.getIn(
...
it('setIn with a.b.c.{aaa:ooo,bbb}', () => {
  expect(
...
it('getIn with a.b.c.{aaa:ooo,bbb}', () => {
  expect(
    Path.getIn(
...
it('setIn with a.b.c.{aaa:ooo,bbb:[ccc,ddd]}', () => {
  expect(
    Path.setIn(
      { a: { b: { c: { kkk: 'ddd' } } } },
...
it('getIn with a.b.c.{aaa:ooo,bbb:[ccc,ddd]}', () => {
  expect(
    Path.getIn(
...
it('deleteIn', () => {
  ...
  expect(
    Path.deleteIn(
      ...
      null as unknown as Parameters<typeof Path.deleteIn>[1],
    ),
...
it('test getIn with invalid value', () => {
  const value = {
...
it('test setIn with invalid value', () => {
  const value = {
    a: 1,
    b: 2,
    array: [null, undefined, { nil: null, undef: undefined }],
...
it('path arguments', () => {
  ...
  expect(
    new Path(undefined as unknown as ConstructorParameters<typeof Path>[0]).segments,
  ).toEqual([])
})
...
it('path methods', () => {
  ...
  expect(() => matchPath.map(() => 0)).toThrowError()
  expect(() => regexPath.map(() => 0)).toThrowError()
  ...
  expect(Path.parse('a.b.c').transform(/[ab]/, (...result) => result)).toEqual(
    ['a', 'b'],
  )
  expect(
    Path.parse('a.b.c').transform(
      '',
      null as unknown as Parameters<Path['transform']>[1],
    ),
  ).toEqual('')
