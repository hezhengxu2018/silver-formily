...
function isAssignable(val: any) {
  return typeof val === 'object' || typeof val === 'function'
}

function isNumberIndex(val: any) {
...
export class Path {
  ...
  match = (pattern: Pattern): boolean => {
    ...
    if (path.isMatchPattern) {
      ...
      if (this.isMatchPattern) {
        if (this.isRegExp) {
          try {
            return this.entire instanceof RegExp && typeof path.entire === 'string'
              ? this.entire.test(path.entire)
              : false
          }
          finally {
  ...
  static parse(path: Pattern = '', base?: Pattern): Path {
    ...
    else if (isFn(path) && path[isMatcher]) {
