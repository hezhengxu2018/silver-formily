...
export class Parser extends Tokenizer {
  ...
  parseDestructorExpression(): DestructorExpressionNode {
    ...
    node.source = this.input
      ...
      .replace(/\[(.*)\]/, (match, expression) => {
        ...
        const operator = /^[+\-*/]/.test(normalized) ? normalized[0] : undefined
        const target = operator ? normalized.slice(1).trim() : normalized
  ...
  parseGroupExpression(parent: Node) {
    ...
    while (this.state.type !== eofTok && this.state.type !== parenRTok) {
      if (this.state.type === commaTok) {
        this.next()
        continue
      }
      if (this.state.type === bangTok) {
        node.isExclude = true
