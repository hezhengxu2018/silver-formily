...
- `dist/` is generated by Vite library mode (`pnpm build`). Treat it as read-only—changes there will be discarded.
...
| `pnpm dev`                  | Local development                                              | `vite build --watch`; re-builds on change, but still writes into `dist/`.        |
