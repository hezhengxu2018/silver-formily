...
it('reaction with shallow equals', () => {
  const obs: any = {
    aa: { bb: 123 },
  }
  define(obs, {
...
it('autorun direct recursive react', () => {
...
it('autorun direct recursive react with if', () => {
...
it('autorun indirect recursive react', () => {
  const obs1 = observable<any>({})
  const obs2 = observable<any>({})
...
it('autorun indirect alive recursive react', () => {
...
it('autorun direct recursive react with head track', () => {
  const obs1 = observable<any>({})
  const obs2 = observable<any>({})
  const fn = vi.fn()
  autorun(() => {
...
it('autorun.memo with observable', () => {
...
it('autorun.memo with observable and effect', async () => {
  const obs1 = observable({
    aa: 0,
  })
  const fn = vi.fn()
...
it('autorun.memo with deps and dispose', () => {
  const obs = observable<any>({
    bb: 0,
    cc: 0,
  })
...
it('autorun.memo with invalid params', () => {
  const obs = observable<any>({
    bb: 0,
  })
  const fn = vi.fn()
...
it('autorun.memo not in autorun', () => {
...
it('autorun.effect dispose when autorun dispose', async () => {
  const obs = observable<any>({
    bb: 0,
  })
  const fn = vi.fn()
...
it('autorun.effect with deps', async () => {
  const obs = observable<any>({
    bb: 0,
    cc: 0,
  })
...
it('autorun.effect with default deps', async () => {
  const obs = observable<any>({
    bb: 0,
  })
  const fn = vi.fn()
...
it('autorun.effect not in autorun', () => {
  expect(() => autorun.effect(() => {})).toThrow()
})

it('autorun.effect with invalid params', () => {
  autorun.effect({} as any)
})

it('autorun dispose in batch', () => {
  const obs = observable({
    value: 123,
  })
  const handler = vi.fn()
...
it('set value by computed depend', () => {
  const obs = observable<any>({})
...
it('delete value by computed depend', () => {
  const handler = vi.fn()
  const obs = observable<{
    a?: {
      b: number
...
it('set Set value by computed depend', () => {
  const handler = vi.fn()
  const obs = observable({
    set: new Set(),
  })
...
it('delete Set by computed depend', () => {
  const handler = vi.fn()
  const obs = observable({
    set: new Set([1]),
  })
...
it('set Map value by computed depend', () => {
  const handler = vi.fn()
  const obs = observable({
    map: new Map(),
  })
...
it('delete Map by computed depend', () => {
  const handler = vi.fn()
  const obs = observable({
    map: new Map([[1, 1]]),
  })
...
it('autorun recollect dependencies', () => {
  const obs = observable<any>({
    aa: 'aaa',
    bb: 'bbb',
    cc: 'ccc',
...
it('reaction recollect dependencies', () => {
  const obs = observable<any>({
    aa: 'aaa',
    bb: 'bbb',
    cc: 'ccc',
