...
describe('compare', () => {
  it('isEqual', () => {
    ...
    expect(isEqual(regexpA, /test/)).toBeTruthy()
...
describe('isEmpty', () => {
  ...
  it('isEmpty', () => {
    ...
    const nonEmptyFunc = function (payload: unknown) {
      console.log(payload)
    }
    ...
    emptyError.message = ''
    expect(isEmpty(emptyError)).toBeTruthy()
    ...
    expect(isEmpty(Symbol('empty'))).toBeFalsy()
  })
})
...
describe('shared Subscribable', () => {
  it('subscribable', () => {
    const cb = vi.fn((payload: Record<string, any>) => payload)
    ...
    const customFilter = (payload: Record<string, any>) => {
      payload.key2 = 'val2'
      return payload
    }
    ...
    const customNotify = vi.fn((payload: Record<string, any>) => {
      console.log(payload)
      return false
    })
...
it('defaults', () => {
  ...
  const defaultDate = /^/
  // @ts-expect-error upstream test mutates regex instances with custom metadata
  defaultDate._name = 'name'
  const date2 = /^/
  expect(defaults(defaultDate, date2)._name).toEqual('name')
})

it('applyMiddleware', async () => {
...
it('applyMiddleware with error', async () => {
  try {
    await applyMiddleware(0, [
      () => {
    ...
    expect(e).toEqual(new Error('this is error'))
  }
})
