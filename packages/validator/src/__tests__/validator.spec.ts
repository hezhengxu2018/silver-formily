...
function hasError(results: any, message?: string) {
  if (!message) {
...
it('empty object validate', async () => {
  const results = await validate({}, { required: true })
  expect(results).toEqual({
    error: ['The field value is required'],
    success: [],
...
it('empty number validate', async () => {
...
it('custom validate results', async () => {
  const results = await validate('', {
    validator() {
...
it('max/maxItems/maxLength/minItems/minLength/min/maximum/exclusiveMaximum/minimum/exclusiveMinimum/len', async () => {
...
it('filter trigger type(unmatch)', async () => {
  expect(
    await validate(
      '',
      {
...
it('filter trigger type(match first validate)', async () => {
  expect(
    await validate(
      '',
      {
...
it('filter trigger type(match multi validate)', async () => {
  expect(
    await validate(
      '',
      {
...
it('validate formats(date)', async () => {
...
it('validate formats(number)', async () => {
...
it('validator return boolean', async () => {
  hasError(
    await validate('123', {
      customBool: true,
...
it('validator template with format', async () => {
  registerValidateMessageTemplateEngine((message) => {
    if (typeof message !== 'string')
      return message
    return message.replace(/<<\s*([\w.]+)\s*>>/g, (_, $0) => {
...
it('validator template with format and scope', async () => {
  registerValidateMessageTemplateEngine((message) => {
    if (typeof message !== 'string')
      return message
    return message.replace(/<<\s*([\w.]+)\s*>>/g, (_, $0) => {
