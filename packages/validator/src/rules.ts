...
function isValidateEmpty(value: any) {
  if (isArr(value)) {
...
function extendSameRules(rules: IRegistryRules, names: Record<string, string>) {
...
const RULES: IRegistryRules = {
  format(value, rule) {
    ...
    if (rule.format) {
      ...
      if (format) {
        if (isFn(format)) {
          return !format(value) ? rule.message : ''
        }
  ...
  enum(value, rule) {
    ...
    return !enums.includes(value) ? rule.message : ''
  },
  const(value, rule) {
    if (isValidateEmpty(value))
      return ''
