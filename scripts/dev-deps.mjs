...
const buildDependencies = packageJson.silverFormily?.tasks?.dev?.buildDependencies ?? []

if (!Array.isArray(buildDependencies) || !buildDependencies.every(item => typeof item === 'string')) {
  console.error('silverFormily.tasks.dev.buildDependencies 必须是字符串数组。')
  process.exit(1)
}
...
if (!uniqueDependencies.length) {
  console.log(`No dev build dependencies configured for ${packageJson.name}. Skipping dev:deps.`)
  process.exit(0)
}

console.log(`Preparing dev dependencies for ${packageJson.name}: ${uniqueDependencies.join(', ')}`)
